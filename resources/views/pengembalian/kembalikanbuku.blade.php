...
<style>
    ...
    .btn-confirm { background: #10B981; color: white; border: none; font-family: inherit; font-size: 16px; }
...
@if($pinjaman)
    ...
    <div class="card-detail">
        <div class="book-cover">
            @if($buku && $buku->foto)
                <img src="{{ asset('storage/buku/' . $buku->foto) }}" alt="Cover" style="width:100%; height:100%; object-fit: cover;">
            @else
                <div style="color: #9CA3AF; text-align: center; font-size: 12px;">NO COVER</div>
            @endif
        </div>
    ...
    <div class="action-group">
        @if($isTerlambat)
            <p style="color: #DC2626; font-size: 13px; text-align: center; max-width: 400px;">
                ⚠️ Kamu terlambat mengembalikan buku. Total denda <strong>Rp {{ number_format($totalDenda, 0, ',', '.') }}</strong> akan dicatat.
            </p>
        @endif

        <form action="{{ route('konfirmasi-pengembalian', $pinjaman->id) }}" method="POST" style="width: 100%; display: flex; justify-content: center;">
            @csrf
