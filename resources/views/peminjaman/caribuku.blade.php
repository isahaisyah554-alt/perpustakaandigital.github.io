...
@section('content')
    ...
    <div class="card">
        ...
        @if($item->stok_buku > 0)
            <p class="status ready">✔ Stok Tersedia ({{ $item->stok_buku }})</p>
            <button onclick="location.href='{{ route('halaman-pinjam', $item->id) }}'">+ Pinjam</button>
        @else
            <p class="status empty">✘ Stok Kosong</p>
            <button disabled style="background: #D1D5DB;">Habis</button>
        @endif
    </div>
@endforeach
    </div>
